...
Vector3 Quaternion::Rotate(Vector3 &thisVector, const float &angle)
{
	...
	float angleRads = (float)(angle *(atan(1) * 4) / 180);
	Quaternion Q1 = normalise();
	Quaternion Q2 = Quaternion((float)cos(angleRads / 2), (float)sin(angleRads / 2) * Q1.M_X(), (float)sin(angleRads / 2) * Q1.M_Y(), (float)sin(angleRads / 2) * Q1.M_Z());
	Quaternion Q3 = Q2.conjugate();
	Quaternion Q4(thisVector);
	Q4 = Q2 * Q4 * Q3;
